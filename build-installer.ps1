...
$DefaultPfx  = Join-Path $WinDir 'heapi-dev.pfx'
$DefaultPass = 'heapi-dev-2026'

if ($PfxPath -eq '')    { $PfxPath = $DefaultPfx }
if ($PfxPass -eq '')    { $PfxPass = $DefaultPass }
if ($env:HEAPI_CERT_PATH) { $PfxPath = $env:HEAPI_CERT_PATH }
if ($env:HEAPI_CERT_PASS) { $PfxPass = $env:HEAPI_CERT_PASS }
...
if (-not (Test-Path $PfxPath)) {
    ...
    $cert = New-SelfSignedCertificate `
        -Subject           'CN=Heapi Dev, O=Marcelo Matzembacher, C=BR' `
...
# --- 7. Assinar exe ---
$exe = Join-Path $BinDir 'heapi.exe'
if ($signtool -and (Test-Path $exe)) {
    WriteStep 'Assinando heapi.exe'
